...
const Home = () => {
    ...
    async function sendToBackend(query) {
        try {
            const response = await fetch('https://chatbot-1-0xmd.onrender.com/chatbot', {
                method: 'POST',
                headers: {
    ...
    return (
        <div className="max-w-screen-2xl md:px-20 fixed h-screen container overflow-hidden">
            <div className=' p-10 h-full bg-white'>
                <div className='input-box overflow-y-scroll'>
                    {/* Render UserCard and AssistantCard for each query-response pair */}
                    { check && <Grittings/>}
